...
void Triad::Read() {
	...
	if (third < t.third) return true;	if (third > t.third) return false;
	return false;
	...
	if ((first == t.first) && (second == t.second) && (third == t.third)) return true;	return false;
	...
	if (second > t.second) return true;
	if (third < t.third) return false;
	if (third > t.third) return true;	return true;
